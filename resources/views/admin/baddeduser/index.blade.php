            ...
            <div class="box-body">
                ...
                <table class="table table-bordered">
                    ...
                    @foreach($users as $user)
                        <tr>
                            <td title="用户编号:{{$user->user->id}}">{{$user->id}}</td>
                            <td title="用户名:{{$user->user->name}}">
                                <a href="{{route('user.index',['user_id'=>$user->user->id])}}">
                                    {{$user->user->name}}</a></td>
                            <td title="{{$user->user->info->nick}}">
                                <a href="{{route('user.index',['user_id'=>$user->user->id])}}">
                                    {{str_limit($user->user->info->nick, 6,'...')}}
                                </a>
                            </td>
                            <td>
                                <img src="11{{$user->user->avatar}}">

                            </td>

                            <td title="积分:{{$user->user->info->coins}}">{{$user->user->info->coins}}</td>
                            <td>
